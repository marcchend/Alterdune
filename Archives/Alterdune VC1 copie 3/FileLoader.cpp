...
// Supprime les espaces autour d'une string
...
// Découpe une ligne selon un séparateur
...
// Convertit un texte en enum Cat
Cat FileLoader::parseCategory(std::string texte) {
    if (texte == "NORMAL")   return N;
    if (texte == "MINIBOSS") return M;
    if (texte == "BOSS")     return B;
...
std::vector<Monster> FileLoader::loadMonsters(std::string cheminFichier,
    ...
    while (std::getline(fichier, ligne)) {
        ...
        Cat    cat       = parseCategory(colonnes[0]);
        ...
        for (int i = 6; i < (int)colonnes.size(); i++) {
            if (colonnes[i] != "-" && !colonnes[i].empty()) {
                ...
                const Action* a = findA(catalogue, colonnes[i]);
                if (a == nullptr) {
